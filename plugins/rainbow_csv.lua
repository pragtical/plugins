...
local function csv_tokenizer(syn, text, state, resume, delimiter)
  ...
  if resume and resume.tokens then
...
function tokenizer.tokenize(incoming_syntax, text, state, options)
  ...
  if incoming_syntax.csv_tokenizer then
    return incoming_syntax.csv_tokenizer(incoming_syntax, text, state, options, incoming_syntax.delimiter)
  end
  -- otherwise, fallback to default tokenizer
  return old_tokenize(incoming_syntax, text, state, options)
end
